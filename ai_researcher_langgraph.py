...
# Step: Setup LLM
model = ChatGoogleGenerativeAI(model="gemini-2.0-flash", api_key=os.getenv("GOOGLE_API_KEY")).bind_tools(tools)
...
Finally, when asked to write the paper, you MUST call the tool 
`render_latex_pdf` with the full LaTeX content of the paper. 
Do NOT output the LaTeX text directly. Always use the tool.Make sure that TEX file is correct and there is no error in it so that PDF is easily exported. When you give papers references, always attatch the pdf links to the paper"""
